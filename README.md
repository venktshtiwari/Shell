...
1. It is split into tokens using `std::stringstream`.
2. The shell checks if the primary command matches any string in the `builtins` vector.
3. If it's a `type` request, it explicitly checks the builtins list first, then manually parses the `PATH` environment variable using `std::getenv("PATH")` and verifies execution permissions via `access(..., X_OK)`.
4. If it's an external command, the shell shifts to system process management.
